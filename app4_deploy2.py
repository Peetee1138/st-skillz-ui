...
#   0.4  - Back out efforts to highlight the heatmap on Tab 1 (failed), add flex sizing to 2-skill tab; update to use pass5_unique data
...
CLASS_ALL_DATA_FILES = {
    "G2": CLASS_DIR / "G2_all_data_pass5_unique.csv",
    "G3": CLASS_DIR / "G3_all_data_pass5_unique.csv",
    "G4": CLASS_DIR / "G4_all_data_pass5_unique.csv",
}

# NEW: NPZ versions (same folder, same base name)
CLASS_ALL_DATA_NPZ_FILES = {
    "G2": CLASS_DIR / "G2_all_data_pass5_unique.npz",
    "G3": CLASS_DIR / "G3_all_data_pass5_unique.npz",
    "G4": CLASS_DIR / "G4_all_data_pass5_unique.npz",
}
...
def get_all_data_for_class(class_code: str):
    """
    Lazy loader for *_all_data_pass5_unique, using NPZ (mr_*) ONLY.
...
ASSESS_RANK_COL = "rank_mx_95ile_r90_avg"   # this is your column T equivalent

def build_skill_order_from_assess(df_assess: pd.DataFrame, skills_in_scope: list[str]) -> list[str]:
    """
    Returns skills ordered EXACTLY by assess rank column, ascending.
    ...
    tmp = tmp.sort_values([ASSESS_RANK_COL, "sk_name"], ascending=[True, True])
...
def build_rating_components_table(row):
    ...
    r1 = row.get("c_r1_p5", np.nan)
    r2 = row.get("c_r2_p5", np.nan)
    r3 = row.get("c_r3_p5", np.nan)

    rows = [
        {
            "title": "Reliability (R1)",
            "line2": "How often the team wins the quest.",
            "line3a": (
                "R1 scores the build by comparing its Quest Success Rate (win %), proportionally"
                "against all other builds of its class."
            ),
            "line3b": "100% win rate = max R1.",
            "val": fmt_score(r1),
        },
        {
            "title": "Survival (R2)",
            ...
            "line3a": (
                "R2 evaluates minimum survival % across all heroes (min_h1h2h3) and compares "
        ...
        {
            "title": "Skill Efficiency (R3)",
            "line2": "How balanced and optimal are the skills contributing to build is for its class.",
            "line3a": (
                "R3 measures how the skill set in the build stacks in terms of how efficiently each skill ",
                "contributes to the hero's stats."
            ),
            "line3b": (
                "This stat is independent of sim, and makes up a sall portion of the overall rating."
            ),
            "val": fmt_score(r3),
        },
    ]
...
def make_layout_tab3():
    return html.Div(
        ...
        children=[
            ...
            #         {"name": "n/q", "id": "nq_pct"},
            #         {"name": "<80", "id": "sub80_pct"},
            #         {"name": "80–95", "id": "pct_80_95"},
            #         {"name": "95+", "id": "pct_95"},
            #     ],
            #     data=[],
...
def update_outputs(class_code, skill1, skill2, skill_filter):
    ...
    sub[["s3", "s4"]] = sub["skill_list"].apply(lambda skills: pd.Series(_extract_s3s4(skills)))
    sub = sub.dropna(subset=["s3", "s4"]).copy()
    ...
    # ---------------------------------------------------------
    # Axis skill order MUST match assess rank for "All"
    # ---------------------------------------------------------
    df_assess = get_single_skill_assess_df(class_code)
    
    # Start from assess rank order (rank_mx_95ile_r90_avg asc), then append any missing
    axis_skills = build_skill_order_from_assess(df_assess, sorted(list(skills_present)))

    # ---------------------------------------------------------
    # Apply dynamic filter to axis_skills (Top 10 / Top 20 / Epic+Rare)
    # ---------------------------------------------------------
    filter_mode = (skill_filter or "all").lower()

    if filter_mode in ("top10", "top20"):
        # Use per-skill assess ranks (rank_mx_95ile_r90_avg), ASCENDING (best first)
        df_assess = get_single_skill_assess_df(class_code)

        if df_assess is not None and not df_assess.empty:
            # Skill id column in assess files
            sk_col = "sk_name" if "sk_name" in df_assess.columns else (
                "skill_code" if "skill_code" in df_assess.columns else None
            )
            ...
            if sk_col and "rank_mx_95ile_r90_avg" in df_assess.columns:
                ...
                df_rank["rank_mx_95ile_r90_avg"] = pd.to_numeric(
                    df_rank["rank_mx_95ile_r90_avg"], errors="coerce"
                )

                # Keep only skills actually present on the heatmap
                ...
                # Only apply if we got enough to form a grid
                if len(top_skills) >= 2:
                    axis_skills = top_skills

    elif filter_mode == "epic_rare":
        # Use rarity map from db_skill_codes.csv
        epic_rare_codes = {
    ...
    # -----------------------------
    # Figure footprint (row height)
    # -----------------------------
    # We can't literally know screen width from the callback, but:
    # - all / epic_rare: "full width" when container takes 100%
    # - top10: smaller footprint (~1/2 width, ~1/3 less height)
    # - top20: medium footprint (~2/3–3/4 width)
    fig_width = 1700   # let outer layout handle total width by default
    fig_height = 700   # base height for many skills
    margin_top = 50    # space for title + tips + icon band

    if filter_mode == "all":
        fig_width = 1700        # container controls 100% width
        fig_height = 700        # comfortable height for full grid
        margin_top = 50         # enough room but not tons of dead air
    elif filter_mode == "epic_rare":
        fig_width = 1250        # container controls 100% width
        fig_height = 675        # comfortable height for full grid
        margin_top = 60         # enough room but not tons of dead air
    elif filter_mode == "top10":
        fig_width  = 900        # feels ~50% of a typical desktop width
        fig_height = 470        # ~33% less than 700 → smaller row height
        margin_top = 75
    elif filter_mode == "top20":
        fig_width  = 1100       # ~66–75% of typical desktop width
        fig_height = 620        # taller than Top10, shorter than All
        margin_top = 65

    # -----------------------------
    # Icon band sizing (row of icons)
    # -----------------------------
    cell_width_units = 1.0
    icon_sizex = 0.7 * cell_width_units  # ~80% of column width
    
    # Base vertical size in "paper" units, scaled by number of skills
    icon_sizey = 0.8 / max(n_skills, 8)
    icon_sizey = min(0.12, max(0.05, icon_sizey))

    # Base vertical position: just above the heatmap (y=1)
    base_icon_y = 1.03  # 2% above the top of the heatmap

    if filter_mode == "all":
        icon_y = base_icon_y          # default gap
    elif filter_mode == "epic_rare":
        icon_y = base_icon_y + 0.01   # a bit more room for big icons
        icon_sizex *= 0.95
    elif filter_mode == "top10":
        icon_y = base_icon_y + 0.04   # a bit more room for big icons
        icon_sizex *= 0.80
    elif filter_mode == "top20":
        icon_y = base_icon_y + 0.02  # slightly more than All
        icon_sizex *= 0.95
    ...
    # Colorbar tweaks (slightly bigger in zoomed modes)
    colorbar_cfg = dict(
        title="Rating",
        tickvals=[50, 75, 100],
        ticktext=["50", 75, 100],
...
def drive_detail_selection(
    ...
    # --- Rank click in the table (works even if we got here via tab switch) ---
...
def route_tabs_and_single_skill(
    ...
    def is_real_click():
        # Dash pattern-matching inputs pass lists; normal buttons pass ints.
        if trigger_val is None:
            return False
        if isinstance(trigger_val, (list, tuple)):
            return any((v or 0) > 0 for v in trigger_val)
        try:
            return int(trigger_val) > 0
        except Exception:
            return False
    ...
    if trigger_id_str == "main-tabs" and trigger_val == "tab-single-skill":
        # If class is set but skill is empty, pick a safe default
        # Prefer current single-skill-class if already set, else fall back to hero_class, else G2
        target_class = detail_class or hero_class or "G2"

        # If skill is already selected, don’t touch it
        # (router shouldn’t stomp user choice)
        # NOTE: we remember single-skill-select via persistence, so this is usually fine.
...
def show_combo_detail(class_code, s1, s2, s3, s4):
    ...
    Uses XX_all_data_pass5_unique via get_all_data_for_class, then feeds
...
def update_detail_histogram(class_code, s1, s2, s3, s4):
    ...
    - Uses CLASS_ALL_DATA_FILES[{class_code}] (e.g. G2_all_data_pass5_unique.csv)
